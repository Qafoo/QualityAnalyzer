...
let Chart = {
    ...
    _drawRows: function (element, scales, leaves) {
        ...
        group
            ...
            .on("mouseover", function (leave) {
                _addClass(d3.select(this), "hover")

                _addClass(d3.select(element).selectAll(".paths"), "hover")
                _addClass(d3.select(element).selectAll(".legends"), "hover")
                _addClass(d3.select(element).selectAll(".source-" + leave.id), "outgoing")
                _addClass(d3.select(element).selectAll(".target-" + leave.id), "incoming")
            })
            .on("mouseout", function (leave) {
                _removeClass(d3.select(this), "hover")

                _removeClass(d3.select(element).selectAll(".paths"), "hover")
                _removeClass(d3.select(element).selectAll(".legends"), "hover")
                _removeClass(d3.select(element).selectAll(".source-" + leave.id), "outgoing")
                _removeClass(d3.select(element).selectAll(".target-" + leave.id), "incoming")
            })
    ...
    _drawLinks: function (element, scales, links, leaves) {
        ...
        var width = element.offsetWidth
        var leaveIndex = _.object(_.pluck(leaves, 'id'), _.range(leaves.length))
        var link = g.selectAll(".link").data(links, function (link, count) {
            return "l_" + link.source + "_" + link.target
        })
        var _getLinkPath = this._getLinkPath
        var linkPath = function (link, count) {
            var from = leaveIndex[link.source]
            var to = leaveIndex[link.target]
            var maxWidth = (width / 3 - 20)

            return _getLinkPath((width * 2 / 3), ((from * 24) + 12), ((to * 24) + 12), maxWidth)
        }
    ...
    _drawLegends: function (element) {
        ...
        var legend = legends.append("g").attr("class", "legend")
        var hoverLegend = legends.append("g").attr("class", "legend hover")
        var width = element.offsetWidth
        var margin = 5.5
        var legendWidth = 150
        var legendCenter = width - margin - (legendWidth / 2)
